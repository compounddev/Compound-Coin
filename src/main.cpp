...
const string strMessageMagic = "compound Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if (pindexBest->nHeight < 25)
    {
      nSubsidy = 94958891.757693062 * COIN;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    nRewardCoinYear_1 = MAX_MINT_PROOF_OF_STAKE; // incorrect calculations fixed at block 1150

    int64_t nRewardCoinYear = 250 * CENT;

    int64_t nSubsidy = 0 * COIN;

    if (pindexBest->nHeight <= 1000)
    {
    int64_t nSubsidy = nRewardCoinYear_1 / 25 ; // 10%
    return nSubsidy + nFees;
    }

    else if (pindexBest->nHeight < 1150)
    {
      nSubsidy = nCoinAge * nRewardCoinYear_1; // 250 %
      return nSubsidy + nFees;
    }  
    
    else if (pindexBest->nHeight < 50000)
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear; // 250 %
      return nSubsidy + nFees;
    }  
            
    else if (pindexBest->nHeight < 100000)
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear * 2 ; // 500%
      return nSubsidy + nFees;
    }  
    ...
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear / 2 ; // 125 %
      return nSubsidy + nFees;
    }  

    else if (pindexBest->nHeight < 1500000)
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear / 4 ; // 62.5 %
      return nSubsidy + nFees;
    }  

    else if (pindexBest->nHeight < 2000000)
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear / 8; // 31.25 %
      return nSubsidy + nFees;
    }  
    
    else if (pindexBest->nHeight < 2500000)
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear / 16; // 15.625 %
      return nSubsidy + nFees;
    }  

    else
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear / 32; // 7.8125 %
      return nSubsidy + nFees;
    }          
...
// compound: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "compound", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
